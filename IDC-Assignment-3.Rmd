...
where
\\
\widehat{y}^{(m)} = \hat{\beta}^{(m)}_0 + \hat{\beta_1}^{(m)}x_1 + ..... + \hat{\beta}_n^{(m)}x_n + \epsilon
$$
...
$$
\widehat{y} = \frac{1}{M} ( \sum^M_{m = 1} \hat{\beta}^{(m)}_0 + \hat{\beta_1}^{(m)}x_1 + ..... + \hat{\beta}_n^{(m)}x_n + \epsilon) \\

\widehat{y} = \frac{1}{M}  \sum^M_{m = 1} \hat{\beta}^{(m)}_0x_0
